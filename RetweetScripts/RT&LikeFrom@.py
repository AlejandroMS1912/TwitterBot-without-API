...
class TwitterBot:
    ...
    def RTandLIKE_from_user(self, user):
        '''
        This method firstly reject the cookies pop-up to avoid future click problems, then search the user inputed page. After
        a quick scroll in that page detects all the tweets and store them in a list that will later be the index from wich being 
        liked and retweeted.
...
UserParameters.RTandLIKE_from_user('Cristiano') # ('username')
